	...
	*/
	function comfirmInfor(){
		...
		if( document.getElementById("submit").innerHTML=='提交'){
	 		...
	 		location.href='../recharge_style/Account.jsp';
	 	}
  }
...
<div id="setmeal-contain"><!-- setmeal-contain 开始-->
	<div id="setmeal-main" ><!-- setmeal-main 开始-->
		...
		<table  class="table-style" style="float:left" >
		  <tr>
		    ...
		    <td class="right-td-style">${mobile }</td>
		  </tr>
		  <tr>
		    <td class="left-td-style" >充值金额：</td>
		    <td class="right-td-style">${selfee }</td>
		  </tr>
		  <tr>
		    <td class="left-td-style">号码归属地：</td>
		    <td class="right-td-style">${place }地区</td>
		   </tr>
		  <tr>
	...
	%>
				...
				<li>
					...
					<div title="商旅套餐<%=msm.getPp_fee() %>元" class="package-info"><p class="package-info-price">￥<strong><%=msm.getPp_fee() %></strong></p><p title="国内主叫国内150分钟" class="package-info-des"><%=msm.getFlag() %><br><%=msm.getBusiness_name() %></p></div>
					<div title="商旅套餐<%=msm.getPp_fee() %>元" class="package-name">商旅套餐<%=msm.getPp_fee() %>元</div>
					<div class="package-action"><a name="" href="javascript:void(0)" onclick="location.href='<%=basePath%>/recharge_style/Account.jsp?mobile=${mobile }&business_id=<%=msm.getBusiness_id() %>&pp_id=<%=msm.getPp_id() %>&selfee=${selfee }'" class="blue-btn add-compare">立即办理</a></div>
